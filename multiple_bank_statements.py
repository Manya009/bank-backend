...
class BankStatementConverter:
    ...
    def check_date(self, df):
        if pd.to_datetime(df['Value Date'].iloc[-1]) < pd.to_datetime(df['Value Date'].iloc[0]):
    ...
    def Single_Bank_statement(self, dfs):
        sheets_oNc_list = []
        for key, value in dfs.items():
            ...
            print(transaction_sheet_df)
            transaction_sheet_df.to_excel(self.writer, sheet_name='Delete kr', index=False)
    ...
    def Multiple_Bank_statement(self, dfs):
        ...
        summary_df_list[0].to_excel(self.writer, sheet_name=sheet_name, index=False)
        summary_df_list[1].to_excel(self.writer, sheet_name=sheet_name, startrow=summary_df_list[0].shape[0] + 2,
                                    index=False)
        summary_df_list[2].to_excel(self.writer, sheet_name=sheet_name,
                                    startrow=summary_df_list[0].shape[0] + summary_df_list[1].shape[0] + 4, index=False)
        summary_df_list[3].to_excel(self.writer, sheet_name=sheet_name,
                                    startrow=summary_df_list[0].shape[0] + summary_df_list[1].shape[0] +
                                             summary_df_list[2].shape[0] + 6, index=False)
        summary_df_list[4].to_excel(self.writer, sheet_name=sheet_name,
                                    startrow=summary_df_list[0].shape[0] + summary_df_list[1].shape[0] +
                                             summary_df_list[2].shape[0] + summary_df_list[3].shape[0] + 8, index=False)
        if num_pairs > 1:
            excel_transaction_sheet_df.to_excel(self.writer, sheet_name='Multiple Transaction', index=False)
            eod_sheet_df.to_excel(self.writer, sheet_name='Multiple EOD Balance', index=False)

        self.Single_Bank_statement(dfs)
    ...
    def start_extraction(self):
        ...
        file_name = "saved_excel/Extracted_statements_file.xlsx"
...
pdf_paths = ["bank_pdfs/sbi1 (1).pdf", "bank_pdfs/SBI bank Mukund Arun Dabir pdf.pdf"]
passwords = ["", ""]
# dates should be in the format dd-mm-yy
start_date = ["01-01-2023", "05-04-2022"]
end_date = ["14-03-2023", "30-03-2023"]
